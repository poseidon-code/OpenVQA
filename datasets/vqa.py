...
class DataSet(BaseDataSet):
    def __init__(self, configuration):
        ...
        frcn_feat_path_list = \
            glob.glob(configuration.FEATS_PATH['train'] + '/*.npz') + \
            glob.glob(configuration.FEATS_PATH['val'] + '/*.npz') + \
            glob.glob(configuration.FEATS_PATH['test'] + '/*.npz')

        # Loading question word list
        stat_ques_list = \
            json.load(open(configuration.RAW_PATH['train'], 'r'))['questions'] + \
            json.load(open(configuration.RAW_PATH['val'], 'r'))['questions'] + \
            json.load(open(configuration.RAW_PATH['test'], 'r'))['questions']

        # Loading answer word list
        # stat_ans_list = \
        #     json.load(open(configuration.RAW_PATH['train-anno'], 'r'))['annotations'] + \
        #     json.load(open(configuration.RAW_PATH['val-anno'], 'r'))['annotations']

        # Loading question and answer list
        self.ques_list = []
        self.ans_list = []

        split_list = configuration.SPLIT[configuration.RUN_MODE].split('+')
        for split in split_list:
            self.ques_list += json.load(open(configuration.RAW_PATH[split], 'r'))['questions']
            if configuration.RUN_MODE in ['train']:
                self.ans_list += json.load(open(configuration.RAW_PATH[split + '-anno'], 'r'))['annotations']
        ...
        # Tokenize
        self.token_to_ix, self.pretrained_emb = self.tokenize(stat_ques_list, configuration.USE_GLOVE)
        self.token_size = self.token_to_ix.__len__()
        print(' ========== Question token vocab size:', self.token_size)

        # Answers statistic
        self.ans_to_ix, self.ix_to_ans = self.ans_stat('datasets/answer_dict.json')
    ...
    def load_ques_ans(self, idx):
        if self.configuration.RUN_MODE in ['train']:
    ...
    def load_img_feats(self, idx, iid):
        ...
        frcn_feat_iter = self.proc_img_feat(frcn_feat_x, img_feat_pad_size=self.configuration.FEAT_SIZE['vqa']['FRCN_FEAT_SIZE'][0])
        ...
        bbox_feat_iter = self.proc_img_feat(
            ...
            img_feat_pad_size=self.configuration.FEAT_SIZE['vqa']['BBOX_FEAT_SIZE'][0]
        )
    ...
    def proc_bbox_feat(self, bbox, img_shape):
        if self.configuration.BBOX_NORMALIZE:
    ...
    def proc_ans(self, ans, ans_to_ix):
        ...
        if self.configuration.LOSS_FUNC in ['kld']:
